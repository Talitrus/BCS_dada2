...
libkey <- read.delim("helper_files/key.txt", header = TRUE)
fnFs <- as.character(unlist(BCS_fnFs))
fnRs <- as.character(unlist(BCS_fnRs))
file_roots <- substr(fnFs,1,nchar(fnFs)-10) # "-10" if .gz, "-8" otherwise
...
file_roots_R <- substr(fnRs,1,nchar(fnRs)-10)
...
file.copy( from = as.character(read.df$fnFs), to = paste0('../consolidated/',read.df$uniqID,'_1.fastq.gz'))
file.copy( from = as.character(read.df$fnRs), to = paste0('../consolidated/',read.df$uniqID,'_2.fastq.gz'))
