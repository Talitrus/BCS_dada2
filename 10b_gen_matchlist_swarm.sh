...
CENTROIDS_FILE=$(mktemp)
ORIG_CENTROIDS="swarm_reps_sorted.fasta"
NSLOTS=$(nproc)

#Remove size info to shorten names for BLAST
sed 's/;size=.*//g' $ORIG_CENTROIDS >${CENTROIDS_FILE} 
...
blastn -db $CENTROIDS_FILE -outfmt '6 qseqid sseqid pident' -out swarm_matchlist.txt -qcov_hsp_perc 80 -perc_identity 84 -query $CENTROIDS_FILE -num_threads $NSLOTS
...
rm $CENTROIDS_FILE #remove temp file created
