...
setwd("~/Documents/Grad/STRI/prototyping/scripts")
...
sample.names.1 <- c('a','b','c','d') #fix this with real sample names later
...
BCS1_out <- filterAndTrim(unlist(BCS_fnFs[1]), BCS1_filtFs, unlist(BCS_fnRs[1]), BCS1_filtRs, rm.phix = TRUE, maxN = 0, maxEE = c(2,2), truncQ = 10, trimLeft = 32, truncLen = c(245,243), multithread = FALSE) #set multithread to TRUE when running in normal R and not RStudio
