...
dada2_vsearch_otutab <- read.delim(file = "FINAL_FOR_LULU_95_otutab.txt", quote = "", row.names = 1) # Load OTU table from DADA2 + VSEARCH
...
matchlist_LULU <- read.delim("matchlist_vs95.txt", stringsAsFactors = FALSE) # generate matchlist.txt with BLAST (preferably?) or VSEARCH
...
saveRDS(curated_LULU, file = "LULU_curation_vs95.rds")
